...
#include "dht.h" //header file for temperature
...
float f_temp =0,f_humid=0;

static void getRoomTemperature(float *pRoomTemperature, float *pRoomHumidity) {
   
     dht_read_float_data(GPIO_ID_PIN(16), &f_humid, &f_temp); 	
    *pRoomTemperature = f_temp;
    *pRoomHumidity = f_humid; 	
}
...
void relayActuate_Callback(const char *pJsonString, uint32_t JsonStringDataLen, jsonStruct_t *pContext) {
...
void aws_iot_task(void *param) {
    ...
    windowActuator.cb = relayActuate_Callback;
    windowActuator.pData = &relayState;
    windowActuator.pKey = "relayState";
    ...
    temperatureHandler.cb = NULL;
    temperatureHandler.pKey = "humidity";
    temperatureHandler.pData = &humidity;
    temperatureHandler.type = SHADOW_JSON_FLOAT;
    ...
    while(NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc) {
        ...
        ESP_LOGI(TAG, "=======================================================================================");
        ESP_LOGI(TAG, "On Device: window state %s", relayState);
        getRoomTemperature(&temperature,&humidity);

        rc = aws_iot_shadow_init_json_document(JsonDocumentBuffer, sizeOfJsonDocumentBuffer);
        if(SUCCESS == rc) {
            rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 3, &temperatureHandler,&humidityHandler,
                                             &windowActuator);
            if(SUCCESS == rc) {
